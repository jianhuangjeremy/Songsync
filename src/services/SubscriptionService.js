...
export class SubscriptionService {
  ...
  static async saveSubscriptionTier(tier) {
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (tier !== SUBSCRIPTION_TIERS.FREE) {
          localStorage.setItem(SUBSCRIPTION_START_DATE_KEY, new Date().toISOString());
        }
      }
    }
  }
  ...
  static async getSubscriptionTier() {
    try {
      ...
      // Fallback for development
      if (typeof window !== 'undefined') {
        return localStorage.getItem(SUBSCRIPTION_KEY) || SUBSCRIPTION_TIERS.FREE;
      }
  ...
  static getSubscriptionConfig(tier) {
    return SUBSCRIPTION_CONFIG[tier] || SUBSCRIPTION_CONFIG[SUBSCRIPTION_TIERS.FREE];
  }
  ...
  static async cleanupOldUsageData() {
    ...
    console.log('Usage data cleanup - keeping data from:', thirtyDaysAgo.toISOString().split('T')[0]);
  }
  ...
  static getStripeConfig() {
    ...
    return {
      baseURL: isDevelopment ? 'http://localhost:3001/api/stripe' : 'https://your-production-api.com/api/stripe',
      publishableKey: isDevelopment ? 'pk_test_mock_publishable_key_123456789' : 'pk_live_your_live_key'
    };
  }
  ...
  static async processPayment(tier, paymentMethod, customerInfo) {
    try {
      ...
      let customer = null;
      if (customerInfo) {
        customer = await this.createStripeCustomer(customerInfo.email, customerInfo.name);
      }
      ...
      // Create subscription
      const priceId = tier === SUBSCRIPTION_TIERS.BASIC ? 'price_mock_basic_plan' : 'price_mock_premium_plan';
  ...
  static async cancelSubscription(subscriptionId) {
    try {
      ...
      const response = await fetch(`${config.baseURL}/subscriptions/${subscriptionId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        }
  ...
  static async updateSubscription(subscriptionId, newTier) {
    try {
      const config = this.getStripeConfig();
      const priceId = newTier === SUBSCRIPTION_TIERS.BASIC ? 'price_mock_basic_plan' : 'price_mock_premium_plan';
  ...
  static async getAvailablePlans() {
    try {
      ...
      return Object.values(SUBSCRIPTION_CONFIG).filter(config => config.id !== 'free');
    }
  }
  ...
  static async getSubscriptionStartDate() {
    try {
      const startDate = await SecureStore.getItemAsync(SUBSCRIPTION_START_DATE_KEY);
